import SalesPage from "@/components/topper/SalesPage";

export const metadata = {
  title: "50+ UPSC Topper Answer Copies + 21 Strategy Guides — Get at ₹11/copy | UPSCPrepNotes",
  description:
    "Complete UPSC preparation compilation with 50+ verified topper answer copies (GS1-4, Essay, Optional) + 21 original strategy guides. Just ₹11 per copy — delivered via email. Pay via UPI.",
  alternates: {
    canonical: "https://upscprepnotes.in/toppers/toppers-copy-compilation",
  },
};

export default function Page() {
  return <SalesPage />;
}
