      ...
      <div className="max-w-md w-full text-center">
        <div className="rounded-2xl border border-border bg-white p-8">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-secondary">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-muted-foreground"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
          </div>
          <h1 className="text-lg font-bold text-foreground">Strategy Report Preview</h1>
          <p className="mt-1 text-sm text-muted-foreground">Select a topper to test</p>
          <div className="mt-6 space-y-2">
            {TOPPERS_FOR_TEST.map(tp => (
              <Link
                key={tp.slug}
                href={`/api/report-preview?slug=${tp.slug}`}
                className="block w-full rounded-lg border border-border px-4 py-3 text-sm font-medium text-foreground/70 hover:border-gray-400 hover:bg-secondary transition-colors"
              >
