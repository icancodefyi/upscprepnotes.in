...
import { getMonth } from "@/lib/current-affairs-utils";
...
export default function CurrentAffairsPage({ slug }: { slug: string }) {
  const data = getMonth(slug);
  if (!data) return null;
  ...
  type Section = { title: string; items: { headline: string; body: string }[] };
  function SectionContent({ s, si }: { s: Section; si: number }) {
    ...
    return (
      <div className="mb-20 last:mb-0 scroll-mt-24" id={`section-${si}`}>
        ...
        {productCta && (
          <div className="mt-4">
            <Link href={productCta.href} className="inline-flex items-center gap-1.5 text-[14px] font-semibold transition hover:opacity-70" style={{ color: INK }}>
              {productCta.label}
            </Link>
          </div>
        )}
        <div className="mt-4">
          <Link href={`/ask?q=${encodeURIComponent(`Ask about: ${s.title} current affairs`)}`}
            className="inline-flex items-center gap-1.5 text-[13px] font-medium transition hover:opacity-70" style={{ color: INK_FAINT }}>
  ...
  return (
    <main style={{ backgroundColor: CANVAS, minHeight: "100vh" }}>
      <div className="mx-auto max-w-6xl px-6 pb-20 pt-8">
        <div className="rounded-xl p-6 sm:p-8" style={{ backgroundColor: SURFACE, border: `2px solid ${INK}` }}>
        ...
        <div className="mt-16 space-y-4">
          <div className="rounded-xl p-6 sm:p-8" style={{ backgroundColor: SURFACE, border: `1px solid ${HAIRLINE}` }}>
            ...
            <Link href="/ask" className="mt-3 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-[14px] font-semibold text-white transition hover:scale-95" style={{ backgroundColor: INK }}>
              Generate Quiz →
            </Link>
          </div>
          ...
          <div className="rounded-xl p-6 sm:p-8 text-center" style={{ backgroundColor: SURFACE, border: `1px solid ${HAIRLINE}` }}>
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              ...
              <Link href={`/api/generate-current-affairs?month=${slug}`} className="text-[13px] font-medium underline-offset-4 hover:underline" style={{ color: INK_MUTED }}>Download PDF</Link>
            </div>
          </div>
        </div>
      </div>
