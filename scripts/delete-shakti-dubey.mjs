// Run: node -r dotenv/config scripts/delete-shakti-dubey.mjs dotenv_config_path=.env.local
...
if (!UPSC_MONGO || !AIRLIST_MONGO) {
  console.error("MONGODB_URI / AIRLIST_MONGO_URI not set. Run with: node -r dotenv/config scripts/delete-shakti-dubey.mjs dotenv_config_path=.env.local");
  process.exit(1);
}
