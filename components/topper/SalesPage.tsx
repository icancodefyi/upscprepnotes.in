...
export default function SalesPage() {
  ...
  async function handleSamples(e: React.FormEvent) {
    e.preventDefault();
    setSError("");
    if (!sEmail.trim()) {
      setSError("Email is required.");
      return;
    }
    setSLoading(true);
    try {
      const r = await fetch("/api/free-guides", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: "", email: sEmail.trim() }),
      });
      if (!r.ok) {
        const d = await r.json();
        throw new Error(d.error || "Failed");
                ...
                <div className="rounded-2xl border-2 border-emerald-200 bg-white p-6 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.1)] sm:p-8">
                  ...
                  {sDone ? (
                    <p className="mt-4 text-center text-xs font-semibold text-emerald-600">
                      Check your inbox — we sent 3 free guides!
                    </p>
                  ) : (
                    <form onSubmit={handleSamples} className="mt-4 flex gap-2">
                      <input
                        type="email"
                        value={sEmail}
                        onChange={(e) => setSEmail(e.target.value)}
                        placeholder="Enter your email"
                        className="min-w-0 flex-1 rounded-lg border border-gray-200 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                      />
                      <button
                        type="submit"
                        disabled={sLoading}
                        className="shrink-0 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-500 disabled:opacity-50 transition-all"
                      >
                        {sLoading ? "Sending..." : "Get 3 Free Guides"}
                      </button>
                    </form>
                  )}
                  {sError && (
                    <p className="mt-2 text-center text-xs text-red-500">{sError}</p>
                  )}
