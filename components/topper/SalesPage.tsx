...
const TOPPERS = [
  {
    name: "Saurabh Sharma", rank: "AIR 23",
    highlight: "136 Marks in Essay", speciality: "Art of Brainstorming",
    takeaway: "Learn the power of connecting phrases from his answer copies",
    color: "bg-zinc-900",
  },
  {
    name: "Animesh Pradhan", rank: "AIR 2",
    highlight: "109 Marks in GS1", speciality: "Art of Presentations",
    takeaway: "Learn using diagrams, micro-diagrams, examples from his answer copies",
    color: "bg-zinc-800",
  },
  {
    name: "Kunal Rastogi", rank: "AIR 15",
    highlight: "134 Marks in GS2", speciality: "Writing Perfect GS2 Answers",
    takeaway: "Learn the art of writing more in limited words with tabular format",
    color: "bg-zinc-900",
  },
  {
    name: "Shaurya Arora", rank: "AIR 14",
    highlight: "101 Marks in GS3", speciality: "Scoring in the Lowest Scoring Paper",
    takeaway: "See how Shaurya used argument and example method in GS3",
    color: "bg-zinc-800",
  },
  {
    name: "Aditya Shrivastava", rank: "AIR 1",
    highlight: "143 Marks in GS4", speciality: "Multi-dimensional Approach",
...
const FEATURE_ROWS = [
  "Keywords", "Facts", "Data", "Maps", "Diagrams",
  "Micro Diagrams", "Crisp Introductions", "Forward Looking Conclusions",
  "Bouncer Questions", "5 Sec Diagram Collections",
];
...
export default function SalesPage() {
  ...
  return (
    <>
      {/* ─── STICKY TOP BAR ─── */}
      <div
        className={`fixed top-0 left-0 right-0 z-40 border-b border-black/[0.06] bg-white/90 backdrop-blur-md transition-all duration-300 ${
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          <p className="text-sm font-medium">
            50+ Topper Copies{" "}
            <span className="text-zinc-400">• Marks Wise</span>
          </p>
          <button
            onClick={() => setModalPkg({ name: "All", price: "999" })}
            className="rounded-full bg-black px-6 py-2 text-xs font-medium text-white transition hover:bg-zinc-800"
          >
            Buy at ₹999 ←
          </button>
        </div>
      ...
      <main className="min-h-screen bg-[#f8f7f4] text-black">
        ...
        <div className="relative mx-auto max-w-7xl px-6 pb-28 pt-10">
          ...
          <section className="mb-28">
            <div className="max-w-5xl">
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <button
                  onClick={() => setModalPkg({ name: "All", price: "999" })}
                  className="rounded-full bg-black px-10 py-4 text-sm font-medium text-white shadow-lg shadow-black/10 transition hover:bg-zinc-800 hover:shadow-xl"
                >
                  Buy Now at ₹999 →
                </button>
              ...
              {/* TRUST BAR */}
              <div className="mt-8 flex flex-wrap gap-8 text-xs text-zinc-400">
          ...
          <section className="mb-28">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {IBEC_STEPS.map((step) => (
                ...
                >
                  ...
                  <h3 className="text-xl font-semibold tracking-tight">Learn the Perfect {step.title}</h3>
            ...
            <div className="mt-10 text-center">
              <button
                onClick={() => setModalPkg({ name: "All", price: "999" })}
                className="rounded-full bg-black px-10 py-4 text-sm font-medium text-white shadow-lg transition hover:bg-zinc-800"
              >
                Start Learning IBEC →
              </button>
            </div>
                ...
                >
                  ...
                  <div className="mt-4 space-y-3">
                    {[
                      ...
                      <div key={text} className="flex items-start gap-2 text-sm leading-6 text-zinc-700">
                        <span className="mt-0.5 text-green-600">✅</span>
                        {text}
                      </div>
                    ))}
          ...
          <section className="mb-28 overflow-hidden rounded-[36px] bg-black px-8 py-14 text-white md:px-16 md:py-20">
            <div className="mx-auto max-w-4xl text-center">
              ...
              <button
                onClick={() => setModalPkg({ name: "All", price: "999" })}
                className="mt-10 rounded-full bg-white px-10 py-4 text-sm font-medium text-black transition hover:bg-zinc-200"
              >
                Get Instant Access →
              </button>
            </div>
          ...
          <section className="mb-28">
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              {PACKAGES.map((pkg) => (
                ...
                >
                  {pkg.popular && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-black px-4 py-1 text-[10px] font-medium uppercase tracking-wider text-white whitespace-nowrap">
                      Best Value
                    </span>
                  )}
                  ...
                  <button
                    onClick={() => setModalPkg({ name: pkg.name, price: pkg.price })}
          ...
          <section className="mb-28 overflow-hidden rounded-[36px] border border-black/[0.06] bg-white p-10 md:p-16">
            <div className="text-center">
              ...
              <div className="mt-10 flex justify-center gap-4">
                <button
                  onClick={() => setModalPkg({ name: "All", price: "999" })}
                  className="rounded-full bg-black px-12 py-4 text-sm font-medium text-white shadow-lg transition hover:bg-zinc-800"
                >
                  BUY NOW — ₹999
                </button>
              </div>
        ...
        {/* ─── FLOATING BOTTOM CTA ─── */}
        <div
          className={`fixed bottom-0 left-0 right-0 z-40 border-t border-black/[0.06] bg-white/90 backdrop-blur-md transition-all duration-300 ${
            scrolled ? "translate-y-0" : "translate-y-full"
          }`}
        >
          <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
            <div>
              ...
              <p className="text-xs text-zinc-400">Marks Wise • IBEC Method • Lifetime Access</p>
            </div>
