"use client";

import { useState } from "react";
import FeedbackDialog from "@/components/FeedbackDialog";

export default function ContactFeedbackSection() {
  const [open, setOpen] = useState(false);

  return (
    <>
      <section className="space-y-5">
        <h2 className="text-2xl font-semibold">Send Feedback</h2>
        <p className="leading-8 text-zinc-700">
          Use the form below to submit feedback, report a bug, suggest a feature,
          or request a correction. We review every submission.
        </p>
        <button
          type="button"
          onClick={() => setOpen(true)}
          data-track="contact-open-feedback"
          className="inline-flex items-center gap-2 rounded-xl bg-zinc-800 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-zinc-700"
        >
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
          </svg>
          Open feedback form
        </button>
      </section>
      <FeedbackDialog open={open} onClose={() => setOpen(false)} />
    </>
  );
}
